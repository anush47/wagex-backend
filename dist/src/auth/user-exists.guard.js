...
let UserExistsGuard = class UserExistsGuard {
    ...
    canActivate(context) {
        const skip = this.reflector.getAllAndOverride(skip_user_check_decorator_1.SKIP_USER_CHECK_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        if (skip)
        ...
        if (user?.isGuest) {
            throw new common_1.ForbiddenException('User registration required');
        }
        return true;
    }
};
...
//# sourceMappingURL=user-exists.guard.js.map